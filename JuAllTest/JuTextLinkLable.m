...
//  JuAllTest
//
//  Created by Juvid on 2018/6/27.
//  Copyright © 2018年 Juvid. All rights reserved.
//

#import "JuTextLinkLable.h"
#import <CoreText/CoreText.h>
@implementation JuTextLinkLable
...
- (void)highlightLinksWithIndex:(CFIndex)index {
    ...
    for (JuLinkModel *model in self.ju_arrLink) {
        NSRange matchRange = model.ju_linkRang;
        if (index>=0) {
            NSLog(@"位置：%d",index);
        }
        if (NSLocationInRange(index,matchRange)) {
            [attributedString addAttribute:NSBackgroundColorAttributeName value:[UIColor colorWithWhite:0 alpha:0.3] range:matchRange];
        }
        else {
...
-(NSMutableArray *)ju_arrLink{
    if (!_ju_arrLink) {
        _ju_arrLink = [NSMutableArray array];
    }
    return _ju_arrLink;
}
@end
